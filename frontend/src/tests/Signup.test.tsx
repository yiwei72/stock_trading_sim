...
describe('Test Signup component', () => {
  it('all input boxes can be filled correctly', () => {
    ...
    const { getByPlaceholderText } = render(<Signup handleLogin={handleLogin} handleLogout={handleLogout} />);
    ...
    fireEvent.change(firstNameInput, { target: { value: 'FirstName', name: 'firstName' } });
    expect(firstNameInput.getAttribute('value')).toBe('FirstName');

    const lastNameInput = getByPlaceholderText('Your Last Name');
    fireEvent.change(lastNameInput, { target: { value: 'LastName', name: 'lastName' } });
  ...
  it('displays an error message when any input is missing', () => {
    const handleLogin = jest.fn();
    const handleLogout = jest.fn();
    const { getByPlaceholderText, getByText } = render(<Signup handleLogin={handleLogin} handleLogout={handleLogout} />);
    ...
    const confirmPasswordInput = getByPlaceholderText('Confirm Password');
    fireEvent.change(confirmPasswordInput, { target: { value: '', name: 'confirmPassword' } });
  
    const firstNameInput = getByPlaceholderText('Your First Name');
    fireEvent.change(firstNameInput, { target: { value: '', name: 'firstName' } });

    const lastNameInput = getByPlaceholderText('Your Last Name');
    fireEvent.change(lastNameInput, { target: { value: '', name: 'lastName' } });
  ...
  it('displays an error message when password and confirm_password do not match', () => {
    ...
    const confirmPasswordInput = getByPlaceholderText('Confirm Password');
    fireEvent.change(confirmPasswordInput, { target: { value: 'wrong', name: 'confirmPassword' } });
